            ...
            if (array_filter($row)) {
                ...
                foreach ($headers as $index => $header) {
                    if (isset($columnMapping[$header])) {
                        // Parse the date of birth if it exists in the row
                        if ($header === 'DateOfBirth' && isset($row[$index])) {
                            $dob = \DateTime::createFromFormat('Y-m-d', $row[$index]);
                            if ($dob) {
                                $teacherData[$columnMapping[$header]] = $dob->format('Y-m-d');
                            } else {
                                continue; // Skip this row if DOB format is incorrect
                            }
                        } else {
