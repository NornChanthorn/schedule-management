...
export default {
  ...
  methods: {
    ...
    async importTeachersFromCSV(event) {
      ...
      try {
        ...
        for (let row of parsedData) {
          ...
          // Call createTeacher method to create the teacher
    ...
    async createTeacher(teacherData) {
      try {
        // Make a POST request to your backend API to create a teacher
        const response = await axios.post("teachers/import", teacherData);

        // Optionally, handle successful creation response
        console.log("Teacher created:", response.data.teachers);
      } catch (error) {
