      ...
      <div class="header_menu">
        <div v-if="user.role === 'admin'" class="mr-6">
          <div class="flex items-center mx-auto">
            <button
              class="flex items-center justify-center w-32 border-blue-500 border text-blue-500 px-2 py-2 hover:bg-blue-100 focus:outline-none mr-2"
              label="Add New" severity="secondary" @click="showDialog">
              <span class="flex items-center mr-8"> Menu </span>
              <i
                :class="{ 'fa-solid fa-chevron-down': !isDialogVisible, 'fa-solid fa-chevron-down rotated': isDialogVisible }"></i>
            </button>
          </div>
          <!-- Modal or dropdown content -->
          <transition name="fade" appear>
            <div v-if="isDialogVisible"
              class="absolute bg-white mt-2 w-32 border-blue-500 border text-blue-500 modal-content">
        ...
        <div v-if="user.role == 'teacher'"
          class="relative mr-2 text-center cursor-pointer flex items-center justify-center flex-col"
          @click="togglePopup">
          <div
            class="notification bg-white absolute top-10 text-black text-lg font-semibold opacity-0 pointer-events-none transition-transform transform -translate-y-full"
            :class="{ 'opacity-100 translate-y-0': showPopup }">
            <div v-if="showPopup" class="notification-list">
              <!-- Loop through your notifications -->
              <div class="h-2 w-full bg-blue-500"></div>
              <div class="px-4 py-2">
                <div v-for="(notification, index) in notifications" :key="index"
                  class="notification-item text-sm text-left">
                  {{ notification }}
                </div>
              </div>
            </div>
            <div class="w-3 h-3 bg-blue-500 absolute top-[-6px] left-1/2 transform -translate-x-1/2 rotate-45 z-[-199]">
            </div>
          </div>
        ...
        <div class="h-10 w-10 mr-2">
          <button class="button" @click="TogglePopup('buttonTrigger')">
            <img src="/img/profile.png" alt="User Avatar"
              class=" rounded-full transition-all border-2 hover:border-blue-500">
          </button>
