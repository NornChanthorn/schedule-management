        ...
        <div class="w-2/3">
          <div class="flex mb-4" v-if="user.role=='teacher'">
            <div class="w-1/2 mr-2">
              ...
              <input type="text" v-model="teacher.f_name"  id="firstName" class="mt-1 p-2 w-full border rounded outline-none focus:outline-blue-200">
            </div>

            <div class="w-1/2 ml-2" >
              <label for="lastName" class="block text-m font-medium text-gray-700">Last Name</label>
              <input type="text" v-model="teacher.l_name" id="lastName" class="mt-1 p-2 w-full border rounded outline-none focus:outline-blue-200">
            </div>
          </div>
          <div class="mb-4" v-else>
            <label for="email" class="block text-m font-medium text-gray-700">Name</label>
            <input type="email" v-model="user.name" id="email" class="mt-1 p-2 w-full border rounded outline-none focus:outline-blue-200">
          </div>
          <div class="mb-4" v-if="user.role=='teacher'">
            <label for="email" class="block text-m font-medium text-gray-700">Gender</label>
            <input type="email" v-model="teacher.gender" id="email" class="mt-1 p-2 w-full border rounded outline-none focus:outline-blue-200">
          </div>
         ...
         <!-- Address -->
          <div class="mb-4" v-if="user.role=='teacher'">
            ...
            <textarea v-model="teacher.dob" id="address" rows="1" class="mt-1 p-2 w-full border rounded outline-none focus:outline-blue-200"></textarea>
          </div>

          <!-- Contact Number -->
          <div class="mb-4" v-if="user.role=='teacher'">
            <label for="contactNumber" class="block text-m font-medium text-gray-700">Contact Number</label>
            <input type="tel" v-model="teacher.phone_num" id="contactNumber" class="mt-1 p-2 w-full border rounded outline-none focus:outline-blue-200">
          </div>

          <div class="flex justify-start mt-6">
            <button @click="cancel" class="w-32 px-4 py-2 mr-2 text-gray-700 rounded  border-2 hover:bg-gray-200">Cancel</button>
            <button @click="save" class="w-32 px-4 py-2 text-white rounded border-2 bg-blue-500 hover:bg-blue-700">Save</button>
          </div>
        </div>
    </div>
